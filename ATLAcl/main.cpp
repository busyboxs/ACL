...
{
    //CPath path;
    //if (!::GetModuleFileName(NULL, CStrBufT(path, MAX_PATH, CStrBufA::AUTO_LENGTH), MAX_PATH))
    //    return false;
    //path.StripPath();
    CString path = "ATLAcl";
...
bool ParseCommandLine(int argc, LPSTR argv[])
{
    for (int i = 1; i < argc; i++)
    {
        LPCSTR szOption = argv[i];
